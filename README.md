...
>
> The top row is the **OES bar**, the header of a foldable realtime block that starts collapsed (`▶ OES`; click the bar or its chevron to unfold to `▼ OES • Tok • Cache • Proc • Net` with the chart below — the fold is remembered). The four compact realtime categories (`Tok`, `Cache`, `Proc`, `Net`, each led by a muted `•`) sit on the same line as the status. The OES glyph is a braille spinner while a tab loads or a session switch is in flight and `×` on a real error (with the message); a ready bar with nothing to say is a bare `OES` — no stray dot, because the tabs already lead with one. While the bar is busy (loading, switching, or an error) the realtime tabs step aside so the message reads clean and rejoin the line the moment it is ready. On a cold start the first line is briefly taken by a one-row `engage` boot line — a braille spinner plus a block fill that completes in about a second — which ends when the first snapshot lands and hands over to the `Loaded. Engage!` success toast. No prompt or output text ever leaves the panel — only the count. Unfolded, the same line carries the category tabs and the **realtime block** sits below: a 4-row braille chart with a row selector on its left (Tok: `avg` / `sum` / `in` / `out` / `tok/s`, cache `read`/`write`, `cpu`/`ram`), each row with a `F` fullscreen placeholder (future). The Tok `avg` row is the bar's old live reading — a gentle 5-sample moving average of the total rate; `sum` and `tok/s` both plot the current `in+out`. Picking a row switches the one shared chart to that series. A compact axis column flanks the chart (`0` at the baseline, the recent max on top, ≤3 chars — `100`, `1k`). `Net` (kbit/s) is estimated from the token flow at ≈ 4 bytes/token — `out` is what the session sends to the model (input + cache), `in` is what streams back (output + reasoning) — a live proxy for traffic, not a socket meter.
>
